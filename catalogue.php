                ...
                <?php
                    ...
                    foreach($sql->fetch() as $col => $movieId):
                        if ($col == 0)  //la premiere colonne correspond à l'id de l'utilisateur
                            continue;
                        $sql = $pdo->prepare("SELECT title, year from movies where idMovie = $movieId limit 1");
                        $sql->execute();
                        ...
                        ?>
                            <figure id="movie<?=$movieId?>" class="movie-tile" onclick="showFilmDetails('<?=$movieTitle?>', '<?=$movieYear?>')">
