...
function showFilmDetails(idUser, idMovie, title, year, genres) {
...
function saveRating(starID, idUser, idMovie) {
    if (starID < nbStars) {
    ...
    xhr.onload = function () {
        ...
        xhrGet.onload = function () { //stockage de la valeur retournee
            if (xhrGet.status === 200) {
                ...
                avgRating.innerHTML = xhrGet.responseText;
            } else {
                alert('An error occurred!');
            }
        };
    ...
    let parameters = 'idUser' + window.encodeURIComponent(idUser) + '&idMovie=' + window.encodeURIComponent(idMovie) + '&rating' + window.encodeURIComponent(rating);
    xhr.send(parameters);
}
